...
public class Form1099ListResponseValueInner extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(Form1099ListResponseValueInner.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!Form1099ListResponseValueInner.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'Form1099ListResponseValueInner' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<Form1099ListResponseValueInner>() {
                @Override
                public void write(JsonWriter out, Form1099ListResponseValueInner value) throws IOException {
                ...
                @Override
                public Form1099ListResponseValueInner read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        Form1099ListResponseValueInner ret = new Form1099ListResponseValueInner();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for Form1099ListResponseValueInner: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return Form1099ListResponseValueInner.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to Form1099ListResponseValueInner
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for Form1099ListResponseValueInner with oneOf schemas: Form1042SListItemResponse, Form1095BListItemResponse, Form1099BaseResponse, Form1099DivListItemResponse, Form1099IntListItemResponse, Form1099KListItemResponse, Form1099MiscListItemResponse, Form1099NecListItemResponse, Form1099RListItemResponse. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of Form1099ListResponseValueInner given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of Form1099ListResponseValueInner
     * @throws IOException if the JSON string is invalid with respect to Form1099ListResponseValueInner
     */
    public static Form1099ListResponseValueInner fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, Form1099ListResponseValueInner.class);
    }

    /**
     * Convert an instance of Form1099ListResponseValueInner to an JSON string
     *
     * @return JSON string
     */
